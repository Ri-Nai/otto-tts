...
def _loadAudio(fileDir, norm):
    if not os.path.exists(fileDir):
        raise FileNotFoundError(f"Audio file not found: {fileDir}")
    ...
    if sampleRate != _targetSR:
        ...
        # 转换
        data = np.interp(
            np.array(range(newLength)), np.linspace(0, newLength - 1, len(data)), data
        )
    # 标准化
    if norm:
...
# --------------------------------------------
class TTSEngine:
    def __init__(self, configFileLoc="./data/settings.json"):
        try:
            # 读取设置文件
            with open(configFileLoc, encoding="utf8") as configFile:
                configuration = json.load(configFile)

            with open(configuration["dictFile"], encoding="utf8") as dictFile:
    ...
    def generate_audio_data(
        ...
        return self.__concatenate(rawData, inYsddMode, pitchMult, speedMult, norm, reverse)
    ...
    def __concatenate(self, rawData, inYsddMode, pitchMult, speedMult, norm, reverse):
        ...
        for i in range(0, len(pronunciations)):
                    ...
                    for word in text.split():
                        ...
                        try:
                            concatenated = np.concatenate(
                                (
                                    concatenated,
                                    _loadAudio(os.path.join(self.__voicePath, word + ".wav"), norm),
                                )
                            )
                        # 如果出现错误
                        except Exception as e:
                            print(f"Error loading audio for word '{word}': {e}")
                            # 加入缺失素材列表
                            if word not in missingPinyin:
                                missingPinyin.append(word)
                            # 以空白音频代替
                            concatenated = np.concatenate(
                                (concatenated, np.zeros(int(_targetSR / 4)))
                            )
            ...
            # 使用原声大碟
                ...
                try:
                    concatenated = np.concatenate(
                        (
                            concatenated,
                            _loadAudio(
                                os.path.join(self.__ysddPath, self.__ysddTable[pronunciations[i][0]] + ".wav"),
                                norm,
                            ),
                        )
                    )
                # 如果出现错误
                    ...
                    print(f"Error loading YSDD audio for '{pronunciations[i][0]}': {e}")
                    # 加入缺失素材列表
        ...
        # 音高偏移
        concatenated = _modifyPitchAndSpeed(
            concatenated, pitchMult, speedMult
        )

        # 倒放
        if reverse:
            concatenated = np.flip(concatenated)

        # 如果缺失拼音，则发出警告
        if len(missingPinyin) != 0:
            print("警告：缺失或未定义{}".format(missingPinyin))
            
        return concatenated, _targetSR
